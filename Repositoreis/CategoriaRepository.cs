...
namespace VHBurguer3.Repositoreis
{
    public class CategoriaRepository : ICategoriaRepository
    {
        ...
        {
            //Cria a consulta inicial na tabela Categoria,mas ainda não execulta nada no banco.
            var consulta = _context.Categoria.AsQueryable();

            //se foi informado um id atual
            //significa que estamos EDITANDOuma categoria existente 
            //não vamos ignorar essa própria categoria na verificação
            if(categoriaIdAtual.HasValue)
            {
                //remove da busca a categoria com esse mesmo ID evita que o sistem aconsidere o proprio registro como duplicado
                consulta = consulta.Where(c => c.CategoriaID != categoriaIdAtual.Value);
            }
            //verifica se existe alguma categoria com o 
